...
const calculateLicenseStatus = (expirationDate, gracePeriodDays = 0) => {
    const now = new Date();
...
router.get('/business/:businessId', auth_1.verifyAuth, async (req, res) => {
    try {
...
router.post('/', auth_1.verifyAuth, async (req, res) => {
    try {
        const { businessId, licenseType, licenseNumber, issuingAuthority, issueDate, expirationDate, renewalUrl, gracePeriodDays, notes, } = req.body;
        // Verify business belongs to user
        const business = await prisma.business.findFirst({
            where: { id: businessId, userId: req.userId },
        });
...
router.put('/:id', auth_1.verifyAuth, async (req, res) => {
    try {
        ...
        const { licenseType, licenseNumber, issuingAuthority, issueDate, expirationDate, renewalUrl, gracePeriodDays, notes, } = req.body;
