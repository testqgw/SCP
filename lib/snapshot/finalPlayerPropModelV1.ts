...
function normalizeRow(input: unknown): SnapshotFinalModelBoardRow | null {
  ...
  return {
    ...
    projectedMinutesFloor: num(row.projected_minutes_floor) ?? num(row.projectedMinutesFloor),
    projectedMinutesCeiling: num(row.projected_minutes_ceiling) ?? num(row.projectedMinutesCeiling),
    ...
    availabilityPercentPlay: num(row.availability_percent_play) ?? num(row.availabilityPercentPlay),
