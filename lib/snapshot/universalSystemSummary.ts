...
export const UNIVERSAL_SYSTEM_SUMMARY_VERSION =
  "2026-04-15-resolved-raw-v21-player-local-manifest-v14-five-player-recovery-61-floor";
...
export const UNIVERSAL_SYSTEM_SUMMARY: SnapshotUniversalSystemSummary = {
  ...
  note:
    "Board-faithful live scoreboard using resolved raw accuracy, player overrides, universal fallback, and the active player-local recovery manifest. This version keeps the Nae'Qwan normalized-name fix, keeps Cole Anthony plus Dejounte Murray as dedicated live overrides, and adds targeted player-local recoveries for Nickeil Alexander-Walker, Desmond Bane, Moses Moody, John Konchar, and Lauri Markkanen. Older universal-only 51.x% walk-forward numbers remain research-only and are not the official board score.",
};
