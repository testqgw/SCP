...
import recentWeaknessRouterV8Rules from "@/lib/snapshot/recentWeaknessRouterV8Rules.json";
import recentWeaknessRouterV9Rules from "@/lib/snapshot/recentWeaknessRouterV9Rules.json";
...
export const RECENT_WEAKNESS_ROUTER_V8_START_DATE_ET = "2025-10-23";
export const RECENT_WEAKNESS_ROUTER_V9_START_DATE_ET = "2025-10-23";
...
export const RECENT_WEAKNESS_ROUTER_V8_VERSION = "recent-weakness-router-v8-2026-04-25";
export const RECENT_WEAKNESS_ROUTER_V9_VERSION = "recent-weakness-router-v9-2026-04-25";

type RecentWeaknessRouterMode = "off" | "v1" | "v2" | "v3" | "v4" | "v5" | "v6" | "v7" | "v8" | "v9";
...
export type RecentWeaknessRouterResult = {
  ...
  version:
    ...
    | typeof RECENT_WEAKNESS_ROUTER_V8_VERSION
    | typeof RECENT_WEAKNESS_ROUTER_V9_VERSION;
};
...
type CompiledRecentWeaknessRouterV2Rule = RecentWeaknessRouterV2Rule & {
  order: number;
  parts: Array<[string, string]>;
};

type RecentWeaknessRouterV2RuleIndex = Map<string, CompiledRecentWeaknessRouterV2Rule[]>;

const RECENT_WEAKNESS_ROUTER_V5_RULES: RecentWeaknessRouterV2Rule[] =
...
const RECENT_WEAKNESS_ROUTER_V8_RULES: RecentWeaknessRouterV2Rule[] =
  recentWeaknessRouterV8Rules as RecentWeaknessRouterV2Rule[];
const RECENT_WEAKNESS_ROUTER_V9_RULES: RecentWeaknessRouterV2Rule[] =
  recentWeaknessRouterV9Rules as RecentWeaknessRouterV2Rule[];
...
const RECENT_WEAKNESS_ROUTER_V2_RULE_INDEX = compileRecentWeaknessRouterV2Rules(RECENT_WEAKNESS_ROUTER_V2_RULES);
const RECENT_WEAKNESS_ROUTER_V3_RULE_INDEX = compileRecentWeaknessRouterV2Rules(RECENT_WEAKNESS_ROUTER_V3_RULES);
...
const RECENT_WEAKNESS_ROUTER_V8_RULE_INDEX = compileRecentWeaknessRouterV2Rules(RECENT_WEAKNESS_ROUTER_V8_RULES);
const RECENT_WEAKNESS_ROUTER_V9_RULE_INDEX = compileRecentWeaknessRouterV2Rules(RECENT_WEAKNESS_ROUTER_V9_RULES);
...
export function getRecentWeaknessRouterRuntimeMeta(): {
  ...
  return {
    ...
    version:
      mode === "v9"
        ? RECENT_WEAKNESS_ROUTER_V9_VERSION
        : mode === "v8"
        ? RECENT_WEAKNESS_ROUTER_V8_VERSION
        : mode === "v7"
    ...
    startDateEt:
      mode === "off"
        ? null
        : mode === "v9"
          ? RECENT_WEAKNESS_ROUTER_V9_START_DATE_ET
        : mode === "v8"
          ? RECENT_WEAKNESS_ROUTER_V8_START_DATE_ET
          : mode === "v7"
...
function compileRecentWeaknessRouterV2Rules(rules: RecentWeaknessRouterV2Rule[]): RecentWeaknessRouterV2RuleIndex {
  const index: RecentWeaknessRouterV2RuleIndex = new Map();
  rules.forEach((rule, order) => {
    const parts = rule.key
      .split("|")
      .map((part) => {
    ...
    if (!firstPart) return;
    const firstPartKey = `${firstPart[0]}=${firstPart[1]}`;
    const bucket = index.get(firstPartKey) ?? [];
    bucket.push({ ...rule, order, parts });
    index.set(firstPartKey, bucket);
  });
  return index;
}

function matchesCompiledRecentWeaknessRouterV2Rule(
  rule: CompiledRecentWeaknessRouterV2Rule,
  features: Record<string, string>,
): boolean {
  return rule.parts.every(([key, value]) => features[key] === value);
}

function findRecentWeaknessRouterV2Rule(
  ruleIndex: RecentWeaknessRouterV2RuleIndex,
  features: Record<string, string>,
): CompiledRecentWeaknessRouterV2Rule | null {
  let bestMatch: CompiledRecentWeaknessRouterV2Rule | null = null;
  for (const [key, value] of Object.entries(features)) {
    const bucket = ruleIndex.get(`${key}=${value}`);
    if (!bucket) continue;
    for (const rule of bucket) {
      if (bestMatch && rule.order > bestMatch.order) break;
      if (matchesCompiledRecentWeaknessRouterV2Rule(rule, features)) {
        bestMatch = rule;
        break;
      }
    }
...
function applyRecentWeaknessRouterV2(
  ...
  const rule = findRecentWeaknessRouterV2Rule(RECENT_WEAKNESS_ROUTER_V2_RULE_INDEX, features);
  if (!rule) return null;
...
function applyRecentWeaknessRouterV3(
  ...
  const features = buildRecentWeaknessRouterV2Features(input, currentSide, currentSource);
  const rule = findRecentWeaknessRouterV2Rule(RECENT_WEAKNESS_ROUTER_V3_RULE_INDEX, features);
  if (!rule) return null;
...
function applyRecentWeaknessRouterV4(
  ...
  const features = buildRecentWeaknessRouterV2Features(input, currentSide, currentSource);
  const rule = findRecentWeaknessRouterV2Rule(RECENT_WEAKNESS_ROUTER_V4_RULE_INDEX, features);
  if (!rule) return null;
...
function applyRecentWeaknessRouterV5(
  input: RecentWeaknessRouterInput,
  currentSide: "OVER" | "UNDER",
  currentSource: SnapshotBoardMarketSource,
): RecentWeaknessRouterResult | null {
  const features = buildRecentWeaknessRouterV2Features(input, currentSide, currentSource);
  const rule = findRecentWeaknessRouterV2Rule(RECENT_WEAKNESS_ROUTER_V5_RULE_INDEX, features);
  if (!rule) return null;
  ...
  return {
    ...
    expert: rule.expert,
    ruleKey: rule.key,
    version: RECENT_WEAKNESS_ROUTER_V5_VERSION,
  };
}

function applyRecentWeaknessRouterV6(
  input: RecentWeaknessRouterInput,
  currentSide: "OVER" | "UNDER",
  currentSource: SnapshotBoardMarketSource,
): RecentWeaknessRouterResult | null {
  const features = buildRecentWeaknessRouterV2Features(input, currentSide, currentSource);
  const rule = findRecentWeaknessRouterV2Rule(RECENT_WEAKNESS_ROUTER_V6_RULE_INDEX, features);
  if (!rule) return null;
  ...
  return {
    ...
    expert: rule.expert,
    ruleKey: rule.key,
    version: RECENT_WEAKNESS_ROUTER_V6_VERSION,
  };
}
...
function applyRecentWeaknessRouterV8(
  input: RecentWeaknessRouterInput,
  currentSide: "OVER" | "UNDER",
  currentSource: SnapshotBoardMarketSource,
): RecentWeaknessRouterResult | null {
  const features = buildRecentWeaknessRouterV2Features(input, currentSide, currentSource);
  const rule = findRecentWeaknessRouterV2Rule(RECENT_WEAKNESS_ROUTER_V8_RULE_INDEX, features);
  if (!rule) return null;
  ...
  return {
    ...
    expert: rule.expert,
    ruleKey: rule.key,
    version: RECENT_WEAKNESS_ROUTER_V8_VERSION,
  };
}

function applyRecentWeaknessRouterV9(
  input: RecentWeaknessRouterInput,
  currentSide: "OVER" | "UNDER",
  currentSource: SnapshotBoardMarketSource,
): RecentWeaknessRouterResult | null {
  const features = buildRecentWeaknessRouterV2Features(input, currentSide, currentSource);
  const rule = findRecentWeaknessRouterV2Rule(RECENT_WEAKNESS_ROUTER_V9_RULE_INDEX, features);
  if (!rule) return null;
  ...
  return {
    ...
    expert: rule.expert,
    ruleKey: rule.key,
    version: RECENT_WEAKNESS_ROUTER_V9_VERSION,
  };
}
...
export function applyRecentWeaknessRouter(input: RecentWeaknessRouterInput): RecentWeaknessRouterResult | null {
  ...
  if (
    (mode === "v5" || mode === "v6" || mode === "v7" || mode === "v8" || mode === "v9") &&
    ...
    const v7Result = applyRecentWeaknessRouterV7(input, v7CurrentSide, v7CurrentSource);
    if (mode === "v7") return v7Result ?? v6Result ?? v5Result;

    const v8CurrentSide = v7Result?.side ?? v7CurrentSide;
    const v8CurrentSource = v7Result?.source ?? v7CurrentSource;
    const v8Result = applyRecentWeaknessRouterV8(input, v8CurrentSide, v8CurrentSource);
    if (mode === "v8") return v8Result ?? v7Result ?? v6Result ?? v5Result;

    const v9CurrentSide = v8Result?.side ?? v8CurrentSide;
    const v9CurrentSource = v8Result?.source ?? v8CurrentSource;
    return applyRecentWeaknessRouterV9(input, v9CurrentSide, v9CurrentSource) ?? v8Result ?? v7Result ?? v6Result ?? v5Result;
  }
  ...
  const v4Result = applyRecentWeaknessRouterV4(input, v4CurrentSide, v4CurrentSource);
  if (mode === "v4") return v4Result ?? v3Result ?? v2Result ?? v1Result;

  const v5CurrentSide = v4Result?.side ?? v4CurrentSide;
  const v5CurrentSource = v4Result?.source ?? v4CurrentSource;
  const v5Result = applyRecentWeaknessRouterV5(input, v5CurrentSide, v5CurrentSource);
  if (mode === "v5") return v5Result ?? v4Result ?? v3Result ?? v2Result ?? v1Result;
  ...
  const v7CurrentSource = v6Result?.source ?? v6CurrentSource;
  const v7Result = applyRecentWeaknessRouterV7(input, v7CurrentSide, v7CurrentSource);
  if (mode === "v7") return v7Result ?? v6Result ?? v5Result ?? v4Result ?? v3Result ?? v2Result ?? v1Result;
  ...
  const v9CurrentSource = v8Result?.source ?? v8CurrentSource;
  return (
    applyRecentWeaknessRouterV9(input, v9CurrentSide, v9CurrentSource) ??
    v8Result ??
    v7Result ??
    v6Result ??
    v5Result ??
